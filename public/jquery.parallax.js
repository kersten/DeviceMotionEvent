(function (jQuery) {
    ...
    function setupPorts(viewport, mouseport) {
        ...
        jQuery.extend(mouseport, {width:mouseport.element.width(), height:mouseport.element.height(), top:offset.top, left:offset.left});
    }
    ...
    function setupLayer(layer, i, mouseport) {
        ...
        var yStuff;
        var cssObject = {};
        layer[i] = jQuery.extend({}, {width:layer[i].element.width(), height:layer[i].element.height()}, layer[i]);
        ...
        jQuery.extend(layer[i], {diffxrat:mouseport.width / (layer[i].width - mouseport.width), diffyrat:mouseport.height / (layer[i].height - mouseport.height), xtravel:xStuff.travel, ytravel:yStuff.travel, xtravelpx:xStuff.travelpx, ytravelpx:yStuff.travelpx, xoffset:xStuff.offset, yoffset:yStuff.offset});
        if (xStuff.travelpx) {
    ...
    function setupLayerContents(layer, i, viewportOffset) {
        ...
        for (var n = 0; n < layer[i].element.children().length; n++) {
            ...
            if (layer[i].content[n].anchor) {
                ...
                jQuery.extend(layer[i].content[n], {posxrat:(layer[i].content[n].x + layer[i].content[n].width / 2) / layer[i].width, posyrat:(layer[i].content[n].y + layer[i].content[n].height / 2) / layer[i].height});
            }
        }
    }
    ...
    jQuery.fn.jparallax = function (options) {
        ...
        return this.each(function () {
            ...
            var timer = {running:false, frame:settings.frameDuration, fire:function (x, y) {
            ...
            if (settings.mouseResponse) {
                $(viewport.element).mousemove(function (mouse) {
            ...
            if (settings.triggerResponse) {
                viewport.element.on("jparallax", function (event, ref) {
    ...
    jQuery.fn.jparallax.settings = {mouseResponse:true, mouseActiveOutside:false, triggerResponse:true, triggerExposesEdges:false, xparallax:true, yparallax:true, xorigin:0.5, yorigin:0.5, xtravel:1, ytravel:1, takeoverFactor:0.65, takeoverThresh:0.002, frameDuration:25};
